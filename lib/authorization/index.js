...
function init (env, ctx) {
  ...
  function apiSecretFromRequest (req) {
    ...
    console.info('gcx apiSecretFromRequest', secret);
  ...
  function authorizeAdminSecret (secret) {
    console.info('gcx authorizeAdminSecret: ', env.enclave.isApiKey(secret));
  ...
  authorization.resolve = async function resolve (data, callback) {
    ...
    console.info('gcx authroization.resolve, data.api_secret=', 
      data.api_secret, "token=", data.token, "data.ip=", data.ip)
    ...
    if (data.api_secret == 'null') { // TODO find what's sending this anomaly
      console.error('gcx no api secret found');
    ...
    if (data.api_secret && authorizeAdminSecret(data.api_secret)) {
      console.info("gcx calling requestSuccedded with data.ip", data.ip);
